...
echo -n "$ECHO_RESP" > expected_echo

PING_REQ=$'GET /ping HTTP/1.1\r\n\r\n'
PING_RESP=$'HTTP/1.1 200 OK\r\nContent-Length: 4\r\n\r\npong'

echo -n "$PING_RESP" > expected_ping

# Send four parallel requests
echo -n "$PING_REQ" | nc 127.0.0.1 $PORT > actual_ping_1 &
echo -n "$ECHO_REQ" | nc 127.0.0.1 $PORT > actual_echo_1 &
echo -n "$PING_REQ" | nc 127.0.0.1 $PORT > actual_ping_2 &
echo -n "$ECHO_REQ" | nc 127.0.0.1 $PORT > actual_echo_2 &

wait

diff expected_ping actual_ping_1
diff expected_ping actual_ping_2
diff expected_echo actual_echo_1
diff expected_echo actual_echo_2